...
import com.izaiqi.platform.core.log.desensitizer.DesensitizeModel;
...
import com.izaiqi.platform.web.common.Constants;
...
import static com.izaiqi.platform.web.common.Constants.POST;
import static com.izaiqi.platform.web.common.Constants.UTF8;
...
public class RequestUtils {
    ...
    public static Map parseParams(BaseParam param) throws BaseAppException {
        return parseParams(param, false);
    }

    public static Map parseParams(BaseParam param, boolean validSign) throws BaseAppException {
        if (ObjectUtil.isNotNull(param)) {
            Map params = param.getParams();
            if (validSign && !validSign(param)) {
                ExceptionHandler.publish(Error.ERROR_PARAMS_SIGN_FAILED.getCodeString());
            }
     ...
     * @return
     */
    public static PageParam parsePage(Map params) {
        if (ObjectUtil.isNotNull(params)) {
            if (ObjectUtil.isNotNull(params.get("pageIndex")) && ObjectUtil.isNotNull(params.get("pageSize"))) {
                Integer pageIndex = ((Number) params.get("pageIndex")).intValue();
                Integer pageSize = ((Number) params.get("pageSize")).intValue();
                return new PageParam(pageIndex, pageSize);
            } else if (ObjectUtil.isNotNull(params.get("pageIndex"))) {
                Integer pageIndex = ((Number) params.get("pageIndex")).intValue();
                return new PageParam(pageIndex);
            }
        }
     ...
     * @return
     */
    private static boolean validSign(BaseParam param) {
        // 提取签名
        if (ObjectUtil.isNotNull(param)) {
